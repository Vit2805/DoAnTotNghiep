...
// C/C++ source code generated on : Thu Jan 22 22:29:45 2026
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("testros2");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("testros2");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"testros2\" **\n");
		...
		rtmSetErrorStatus(testros2_M, (NULL));
        testros2_initialize();
		/* External mode */
		rtSetTFinalForExtMode(&rtmGetTFinal(testros2_M));
		rtExtModeCheckInit(1);
		{
			boolean_T rtmStopReq = false;
			rtExtModeWaitForStartPkt(testros2_M->extModeInfo, 1,
				&rtmStopReq);
			if (rtmStopReq) {
				rtmSetStopRequested(testros2_M, true);
			}
		}
...
boolean_T NodeInterface::getStopRequestedFlag(void) {
    ...
    return (!(rtmGetErrorStatus(testros2_M) 
        == (NULL)));
    #else
    return (!(rtmGetErrorStatus(testros2_M) 
        == (NULL)) || rtmGetStopRequested(testros2_M));
    #endif
}
...
void NodeInterface::baseRateTask(void) {
  mRunModel = (rtmGetErrorStatus(testros2_M) ==
              (NULL));
  while (mRunModel) {
    mBaseRateSem.wait();
#ifdef MW_DEBUG_LOG
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModePauseIfNeeded(testros2_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testros2_M, true);
      }
      if (rtmGetStopRequested(testros2_M) == true) {
...
void NodeInterface::extmodeBackgroundTask(void)
{
  while (mRunModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testros2_M->extModeInfo, 1,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testros2_M, true);
      }
